...
} from "../events/leave.publisher.js";
import {
  getEmployeeDetail,
  getEmployeeDetails,
...
export const getLeaveBalance = async (req: AuthRequest, res: Response) => {
  ...
  const balances = await leaveService.getLeaveBalanceByYear(
    employeeId,
    currentYear,
  );
...
export const getPendingLeaveRequests = async (
  ...
  const employeeIds = Array.from(
    new Set(result.leaves.map((leave) => String(leave.employeeId))),
  );
  ...
  const leavesWithDetails = result.leaves.map((leave) => {
    const leaveObj =
      typeof leave.toObject === "function" ? leave.toObject() : leave;
    return {
      ...leaveObj,
